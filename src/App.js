...
function App() {
  ...
  return (
    <div className="App">
      ...
      {/* 컴포넌트화 하기
        컴포넌트 만드는 법
        1. function 만들기 - function App() 바깥에 만들기
        2. return() 안에 html 담기 (하나의 컴포넌트는 하나의 태그로 감싸져 있어야 함. 보통은 div태그로 감싸줌)
        3. <함수명></함수명> 형태로 불러오기

        <div className="modal">
          <h4>제목</h4>
          <p>날짜</p>
          <p>상세내용</p>  모달창 : 클릭 시 상세내용이 나오는 창. 보통은 display:none으로 숨겨져 있다가, 클릭 시 display:block으로 변경되어 나타남. 
        </div>
      */}
      <Modal></Modal>
...
{/* 컴포넌트는언제 쓰면 좋을까?
    1. 반복적인 html 축약할 때
    2. 큰 페이지들 나눌 때 (ex. header, footer, main 등)
    3. 자주 변경되는 부분들 묶어서 관리할 때 (ex. 게시글 하나하나를 컴포넌트로 만들어서 관리하면, 게시글이 변경될 때마다 해당 컴포넌트만 리렌더링되기 때문에, 성능이 좋아짐)    
  */}

{/*
  컴포넌트 만들 때 주의할 점
  state 가져다쓸 때 문제가 됨 (A함수에 있던 변수는 B함수에서 마음대로 가져올 수 없음 )
    */}
export default App;
